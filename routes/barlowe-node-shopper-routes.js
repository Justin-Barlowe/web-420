...
router.post('/customers', async (req, res) => {
  try {
    ...
    res.status(200).json({ message: 'Customer added to MongoDB', data: customer });
  } catch (error) {
...
router.post('/customers/:username/invoices', async (req, res) => {
  try {
    console.log(`Querying username: ${req.params.username}`); // Debug line
    ...
    await customer.save();
    res.status(200).json({ message: 'Invoice added to MongoDB', data: newInvoice });
  } catch (error) {
    console.error(error); // Debug line
...
router.get('/customers/:username/invoices', async (req, res) => {
  try {
    const customer = await Customer.findOne({ userName: req.params.username });
    if (customer) {
      res.status(200).json({ message: 'Invoices retrieved successfully', data: customer.invoices });
    } else {
